...
const originalXCentre = window.innerWidth / 2; 
const nodeXCentre = originalXCentre; 
...
for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 6; j++) {
        ...
        let x = nodeXCentre + r[i] * Math.cos(angle); 
        let y = yCentre + r[i] * Math.sin(angle);
        x = Math.round(x - leftOffset); 
        y = Math.round(y);
...
for (let i = 1; i <= 18; i++) {
    if (i % 6 === 0) {
        const xa = (Node[i][0] + Node[i - 5][0]) / 2 + leftOffset; 
    ...
    if ([1, 5, 3, 12, 10, 8].includes(i)) {
        const xa = (Node[i][0] + Node[i + 6][0]) / 2 + leftOffset; 
    ...
    if (i % 6 !== 0) {
        const xa = (Node[i][0] + Node[i + 1][0]) / 2 + leftOffset; 
